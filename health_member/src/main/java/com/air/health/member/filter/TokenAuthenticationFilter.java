package com.air.health.member.filter;
...
public class TokenAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException, JwtException {
        if ("/member/login".equals(request.getRequestURI()) || "/member/save".equals(request.getRequestURI())) {
        ...
        if (request.getHeader(tokenProvider.getDefaultHeader()) != null) {
            ...
            Claims claims = tokenProvider.parseToken(Constants.TOKEN_MEMBER, token);
            if (claims == null){
                throw new JwtException("token 异常");
            }
            //判断jwt是否过期
            ...
            String data = (String) redisTemplate.opsForValue().get(String.format(Constants.REDIS_KEY_PREFIX_TOKEN_MEMBER, claims.getSubject()));
            if (data == null) {
                throw new AirException("登陆异常, 请重新登录");
            } else {
                MemberEntity member = JSON.parseObject(data, MemberEntity.class);
                UsernamePasswordAuthenticationToken authenticationToken = new UsernamePasswordAuthenticationToken(member.getUsername(), null, member.getAuthorities());
                SecurityContextHolder.getContext().setAuthentication(authenticationToken);
                filterChain.doFilter(request, response);
            }
        } else {
